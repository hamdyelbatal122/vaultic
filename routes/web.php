...
foreach ($routeChannels as $channel => $channelConfig) {
    ...
    Route::middleware((array) ($channelConfig['middleware'] ?? []))
        ...
        ->group(function () use ($channel, $channelConfig, $throttleMiddleware): void {

            // Authenticated routes (registration, deletion, rename)
            Route::middleware(array_filter((array) ($channelConfig['authenticated_middleware'] ?? [])))
                ->group(function () use ($channel, $throttleMiddleware): void {
                    ...
                    if ($channel === 'web') {
                        Route::patch('/{passkey}', [WebAuthnController::class, 'rename'])
